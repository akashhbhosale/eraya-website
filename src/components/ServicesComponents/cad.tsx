...
const CAD = () => {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden text-white bg-black py-6 px-6 mt-0">
      ...
      <div className="relative z-10 max-w-[90rem] w-full mx-auto bg-[url('/assets/cad-bg.png')] bg-cover bg-center bg-no-repeat backdrop-blur-xl border border-black/10 pt-6 pb-12 px-6 rounded-3xl shadow-2xl space-y-8 mb-20">
        <div className="absolute inset-0 bg-white/60 rounded-3xl z-0" />
        ...
        <div className="relative z-10">
          ...
          >
            <h1 className="text-2xl sm:text-4xl md:text-6xl lg:text-7xl font-extrabold text-blue-800 drop-shadow-2xl mb-10 leading-tight tracking-wide text-center">
              Computer Aided Design (CAD)
            </h1>

            <div className="overflow-visible">
              <motion.p className="text-xl md:text-2xl italic font-bold text-blue-800 flex flex-wrap justify-center gap-1 mt-6 leading-[1.5] whitespace-pre-wrap">
                {words.map((word, i) => (
                  <motion.span
                    key={i}
          ...
          <motion.div
            className="text-lg font-semibold text-gray-700 leading-relaxed mt-6 text-center md:text-left"
            data-aos="fade-up"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
          >
            At <span className="font-bold text-black">ERAYA</span>, our
          ...
          <motion.div
            className="grid grid-cols-1 md:grid-cols-3 gap-x-10 gap-y-10 mt-10 items-start justify-center text-center md:text-left"
            data-aos="fade-up"
            initial={{ opacity: 0 }}
          ...
          <div
            className="flex flex-col md:flex-row items-center justify-center text-center md:text-left gap-6 mt-16 transition-transform duration-300 hover:scale-[1.01] hover:bg-black/5 rounded-xl p-4 w-full"
            data-aos="fade-up"
          >
            <div className="w-full md:w-1/2">
              <h3 className="text-2xl font-bold mb-3 text-blue-800 transition duration-300">
                Deliverables
              </h3>
              <ul className="list-disc list-inside space-y-2 text-gray-700 text-base md:text-lg font-semibold">
                {[
                  ...
                  <li
                    key={i}
                    className="hover:text-gray-900 hover:scale-[1.02] transform origin-left transition duration-300"
                  >
                    {item}
                  </li>
                ))}
...
const Section: React.FC<SectionProps> = ({ title, data }) => {
  return (
    <div className="flex flex-col h-full group hover:scale-105 transition duration-300 ease-in-out text-center md:text-left">
      ...
      <ul className="list-disc list-inside pl-2 text-gray-700 text-base md:text-lg font-semibold space-y-2">
        {data.map((item, i) => (
          <li key={i} className="hover:text-gray-900 transition duration-200">
            {item}
          </li>
        ))}
      </ul>
